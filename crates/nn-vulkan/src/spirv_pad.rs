...
//! SPIR-V binary generation for 1D and 2D padding compute shaders.
//!
//! Padding is a fundamental operation for convolution preprocessing, adding
//! constant values around tensor boundaries.
//!
//! # 1D Padding
//!
//! - **Binding 0** (set 0): Input `float[N]` (readonly)
//! - **Binding 1** (set 0): Output `float[N + pad_left + pad_right]`
//! - **Push constants**: `{ uint n, uint pad_left, uint pad_right, float pad_value }`
//!
//! # 2D Padding
//!
//! - **Binding 0** (set 0): Input `float[H * W]` (readonly)
//! - **Binding 1** (set 0): Output `float[(H + pad_top + pad_bottom) * (W + pad_left + pad_right)]`
//! - **Push constants**: `{ uint h, uint w, uint pad_top, uint pad_bottom, uint pad_left,
//!   uint pad_right, float pad_value }`
//!
//! # Dispatch
//!
...
/// Default workgroup size for padding kernels (1D dispatch).
pub const PAD_WORKGROUP_SIZE: u32 = 256;
...
// ============================================================================
// 1D Padding SPIR-V generation
// ============================================================================

/// Generate a SPIR-V 1.0 binary module for 1D padding.
///
/// Each output element is either the pad_value (in the padding region) or
/// a copy of the corresponding input element.
///
/// # Layout
///
/// - **Binding 0** (set 0): Input buffer `float[n]` (readonly)
/// - **Binding 1** (set 0): Output buffer `float[n + pad_left + pad_right]`
/// - **Push constants**: `{ uint n, uint pad_left, uint pad_right, float pad_value }`
///
/// # Returns
///
/// SPIR-V binary as a `Vec<u32>` word vector.
pub fn generate_pad_spirv(n: u32, _pad_left: u32, _pad_right: u32, _pad_value: f32) -> Vec<u32> {
    ...
    // Push constant struct: 3 x uint + 1 x float = 4 members.
    let pc_members = vec![ty_uint, ty_uint, ty_uint, ty_float];
    ...
    b.execution_mode_local_size(func_id, PAD_WORKGROUP_SIZE, 1, 1);
    ...
    // Load push constants: n, pad_left, pad_right, pad_value
    let pc_n_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_0u]);
    let pc_pl_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_1u]);
    let pc_pr_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_2u]);
    let pc_pv_ptr = b.access_chain(ptr_pc_float, var_pc, &[const_3u]);

    let dim_n = b.load(ty_uint, pc_n_ptr);
    let dim_pl = b.load(ty_uint, pc_pl_ptr);
    let dim_pr = b.load(ty_uint, pc_pr_ptr);
    let dim_pv = b.load(ty_float, pc_pv_ptr);

    // total_output = n + pad_left + pad_right
    let n_plus_pl = b.iadd(ty_uint, dim_n, dim_pl);
    let total_output = b.iadd(ty_uint, n_plus_pl, dim_pr);

    // Bounds check: gid_x >= total_output -> return
    ...
    // Check if gid_x is in the input region: pad_left <= gid_x < pad_left + n
    let in_region_start = b.u_greater_than_equal(ty_bool, gid_x, dim_pl);
    let end_of_input = b.iadd(ty_uint, dim_pl, dim_n);
    let in_region_end = b.u_less_than(ty_bool, gid_x, end_of_input);

    // Nested selection for AND of bounds checks
    let copy_label = b.id();
    let check_end_label = b.id();
    let pad_label = b.id();
    let merge_label = b.id();

    b.selection_merge(merge_label);
    b.branch_conditional(in_region_start, check_end_label, pad_label);

    b.label_with_id(check_end_label);
    b.selection_merge(merge_label);
    b.branch_conditional(in_region_end, copy_label, pad_label);

    // Copy path: output[gid_x] = input[gid_x - pad_left]
    b.label_with_id(copy_label);
    let src_idx = b.isub(ty_uint, gid_x, dim_pl);
    let in_ptr = b.access_chain(ptr_sb_float, var_input, &[const_0u, src_idx]);
    let in_val = b.load(ty_float, in_ptr);
    let out_ptr_copy = b.access_chain(ptr_sb_float, var_output, &[const_0u, gid_x]);
    b.store(out_ptr_copy, in_val);
    b.branch(merge_label);

    // Pad path: output[gid_x] = pad_value
    b.label_with_id(pad_label);
    let out_ptr_pad = b.access_chain(ptr_sb_float, var_output, &[const_0u, gid_x]);
    b.store(out_ptr_pad, dim_pv);
    b.branch(merge_label);
...
// ============================================================================
// 2D Padding SPIR-V generation
// ============================================================================

/// Generate a SPIR-V 1.0 binary module for 2D padding.
///
/// Each output element is either the pad_value (in the padding region) or
/// a copy of the corresponding input element.
///
/// # Layout
///
/// - **Binding 0** (set 0): Input buffer `float[H * W]` (readonly)
/// - **Binding 1** (set 0): Output buffer `float[out_h * out_w]`
/// - **Push constants**: `{ uint h, uint w, uint pad_top, uint pad_bottom,
///   uint pad_left, uint pad_right, float pad_value }`
///
/// # Returns
///
/// SPIR-V binary as a `Vec<u32>` word vector.
pub fn generate_pad2d_spirv(
    h: u32,
    w: u32,
    _pad_top: u32,
    _pad_bottom: u32,
    ...
    // Push constant struct: 6 x uint + 1 x float = 7 members.
    let pc_members = vec![
        ty_uint, ty_uint, ty_uint, ty_uint, ty_uint, ty_uint, ty_float,
    ];
    let ty_struct_pc = b.type_struct(&pc_members);
    b.decorate(ty_struct_pc, DECORATION_BLOCK, &[]);
    for i in 0..7u32 {
    ...
    // Load push constants: h, w, pad_top, pad_bottom, pad_left, pad_right, pad_value
    let pc_h_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_0u]);
    let pc_w_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_1u]);
    let pc_pt_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_2u]);
    let pc_pb_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_3u]);
    let pc_pl_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_4u]);
    let pc_pr_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_5u]);
    let pc_pv_ptr = b.access_chain(ptr_pc_float, var_pc, &[const_6u]);

    let dim_h = b.load(ty_uint, pc_h_ptr);
    let dim_w = b.load(ty_uint, pc_w_ptr);
    let dim_pt = b.load(ty_uint, pc_pt_ptr);
    let dim_pb = b.load(ty_uint, pc_pb_ptr);
    let dim_pl = b.load(ty_uint, pc_pl_ptr);
    let dim_pr = b.load(ty_uint, pc_pr_ptr);
    let dim_pv = b.load(ty_float, pc_pv_ptr);

    // out_h = h + pad_top + pad_bottom
    let out_h = b.iadd(ty_uint, dim_h, dim_pt);
    let out_h = b.iadd(ty_uint, out_h, dim_pb);

    // out_w = w + pad_left + pad_right
    let out_w = b.iadd(ty_uint, dim_w, dim_pl);
    let out_w = b.iadd(ty_uint, out_w, dim_pr);

    // total_output = out_h * out_w
    let total_output = b.imul(ty_uint, out_h, out_w);
    ...
    // Decompose gid_x into (out_row, out_col)
    let out_col = b.u_mod(ty_uint, gid_x, out_w);
    let out_row = b.u_div(ty_uint, gid_x, out_w);

    // Check bounds: pad_top <= out_row < pad_top + h AND pad_left <= out_col < pad_left + w
    let row_ge_pt = b.u_greater_than_equal(ty_bool, out_row, dim_pt);
    let end_row = b.iadd(ty_uint, dim_pt, dim_h);
    let row_lt_end = b.u_less_than(ty_bool, out_row, end_row);

    let col_ge_pl = b.u_greater_than_equal(ty_bool, out_col, dim_pl);
    let end_col = b.iadd(ty_uint, dim_pl, dim_w);
    let col_lt_end = b.u_less_than(ty_bool, out_col, end_col);

    // Nested selection: check all four conditions
    let check_row_end = b.id();
    let check_col_start = b.id();
    let check_col_end = b.id();
    let copy_label = b.id();
    let pad_label = b.id();
    let merge_label = b.id();

    b.selection_merge(merge_label);
    b.branch_conditional(row_ge_pt, check_row_end, pad_label);

    b.label_with_id(check_row_end);
    b.selection_merge(merge_label);
    b.branch_conditional(row_lt_end, check_col_start, pad_label);

    b.label_with_id(check_col_start);
    b.selection_merge(merge_label);
    b.branch_conditional(col_ge_pl, check_col_end, pad_label);

    b.label_with_id(check_col_end);
    b.selection_merge(merge_label);
    b.branch_conditional(col_lt_end, copy_label, pad_label);

    // Copy path: output[gid_x] = input[(out_row - pad_top) * w + (out_col - pad_left)]
    b.label_with_id(copy_label);
    let src_row = b.isub(ty_uint, out_row, dim_pt);
    let src_col = b.isub(ty_uint, out_col, dim_pl);
    let src_row_offset = b.imul(ty_uint, src_row, dim_w);
    let src_idx = b.iadd(ty_uint, src_row_offset, src_col);
    let in_ptr = b.access_chain(ptr_sb_float, var_input, &[const_0u, src_idx]);
    let in_val = b.load(ty_float, in_ptr);
    let out_ptr_copy = b.access_chain(ptr_sb_float, var_output, &[const_0u, gid_x]);
    b.store(out_ptr_copy, in_val);
...
/// Compute 1D padding on CPU for reference/verification.
///
/// # Returns
///
/// A vector of length `input.len() + pad_left + pad_right`.
pub fn pad_reference(input: &[f32], pad_left: usize, pad_right: usize, pad_value: f32) -> Vec<f32> {
    let out_len = input.len() + pad_left + pad_right;
    let mut output = vec![pad_value; out_len];
...
/// Compute 2D padding on CPU for reference/verification.
///
/// Input is row-major `[h, w]`. Output is `[h + pad_top + pad_bottom, w + pad_left + pad_right]`.
///
/// # Panics
///
/// Panics if `input.len() != h * w`.
pub fn pad2d_reference(
    input: &[f32],
    h: usize,
    w: usize,
    pad_top: usize,
    ...
    assert_eq!(input.len(), h * w, "input size mismatch");
    let out_h = h + pad_top + pad_bottom;
    let out_w = w + pad_left + pad_right;
    let mut output = vec![pad_value; out_h * out_w];
    for row in 0..h {
        for col in 0..w {
            let out_row = row + pad_top;
            let out_col = col + pad_left;
            output[out_row * out_w + out_col] = input[row * w + col];
        }
    }
    output
}
