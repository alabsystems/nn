...
//! SPIR-V binary generation for where/select and absolute value compute shaders.
//!
...
//! - [`generate_abs_spirv`] — Absolute value.
//!   `output[i] = |input[i]|`
//!
//! All shaders use:
...
//!
//! **Where/Select:**
//! - Binding 0 (set 0): Condition buffer `uint[]` (readonly, 0 or nonzero)
...
/// Default workgroup size for where/abs kernels (1D dispatch).
pub const WHERE_WORKGROUP_SIZE: u32 = 256;
...
pub fn generate_where_spirv(_n: u32) -> Vec<u32> {
    ...
    b.execution_mode_local_size(func_id, WHERE_WORKGROUP_SIZE, 1, 1);
...
/// Generate a SPIR-V 1.0 binary module for element-wise absolute value.
///
/// Computes: `output[i] = |input[i]|` using GLSL.std.450 FAbs.
///
/// # Buffers
///
/// - Binding 0 (set 0): Input buffer `float[]` (readonly)
...
/// * `_n` — Number of elements (reserved for future specialization).
#[must_use]
pub fn generate_abs_spirv(_n: u32) -> Vec<u32> {
    let mut b = SpirVBuilder::new();
    let func_id = b.id();
    ...
    // result = abs(x) via GLSL.std.450 FAbs.
    let result = b.ext_inst(ty_float, glsl_ext, GLSL_STD_450_FABS, &[x]);
...
/// CPU reference implementation for element-wise absolute value.
///
/// Returns `|input[i]|` for each element.
#[must_use]
pub fn abs_reference(input: &[f32]) -> Vec<f32> {
    input.iter().map(|&x| x.abs()).collect()
}

#[cfg(test)]
#[path = "spirv_where_tests.rs"]
mod tests;
