...
//! SPIR-V binary generation for fused linear + activation compute shaders.
//!
//! Generates SPIR-V 1.0 binary modules for fused linear + activation:
//!
//! - [`generate_fused_linear_relu_spirv`]: Linear + ReLU.
//! - [`generate_fused_linear_gelu_spirv`]: Linear + GELU (tanh approximation).
//! - [`generate_fused_linear_silu_spirv`]: Linear + SiLU/Swish.
//!
//! Each kernel computes one output element per thread:
//!
//! ```text
//! output[row, col] = Activation(sum_k(input[row, k] * weight[col, k]) + bias[col])
//! ```
//!
//! Weight is stored in `[out_features, in_features]` layout (transposed relative to
//! input), which is standard for `nn.Linear`.
//!
//! # Buffer layout
//!
//! - Binding 0: Input `[batch_size, in_features]` (readonly)
//! - Binding 1: Weight `[out_features, in_features]` (readonly)
//! - Binding 2: Bias `[out_features]` (readonly)
...
/// Workgroup size for fused linear + activation kernels (1D dispatch).
pub const FUSED_LINEAR_ACT_WORKGROUP_SIZE: u32 = 64;
...
/// Activation variant for fused linear kernels.
...
/// Generate a SPIR-V 1.0 binary for fused linear + activation.
///
/// Internal implementation shared by all three activation variants.
fn generate_fused_linear_act_inner(activation: ActivationKind) -> Vec<u32> {
    ...
    b.execution_mode_local_size(func_id, FUSED_LINEAR_ACT_WORKGROUP_SIZE, 1, 1);
    ...
    let pc_bs_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_0u]);
    let batch_size = b.load(ty_uint, pc_bs_ptr);
    ...
    let cmp_oob = b.u_greater_than_equal(ty_bool, gid_x, total);
    let label_body = b.id();
    let label_exit = b.id();
    b.selection_merge(label_exit);
    b.branch_conditional(cmp_oob, label_exit, label_body);

    b.label_with_id(label_body);

    // row = gid_x / out_features, col = gid_x % out_features
    let row = b.udiv(ty_uint, gid_x, out_features);
    let col = b.umod(ty_uint, gid_x, out_features);

    // ---- Reduction loop: acc = sum_k(input[row, k] * weight[col, k]) ----
    ...
    // phi for k and acc
    let phi_k = b.phi(ty_uint, &[(const_0u, label_body)]);
    let phi_acc = b.phi(ty_float, &[(const_zero_f, label_body)]);
    ...
    // input_idx = row * in_features + k
    let input_offset = b.imul(ty_uint, row, in_features);
    let input_idx = b.iadd(ty_uint, input_offset, phi_k);
    let ptr_in = b.access_chain(ptr_sb_float, var_input, &[const_0u, input_idx]);
    let in_val = b.load(ty_float, ptr_in);

    // weight_idx = col * in_features + k
    let weight_offset = b.imul(ty_uint, col, in_features);
    let weight_idx = b.iadd(ty_uint, weight_offset, phi_k);
    let ptr_w = b.access_chain(ptr_sb_float, var_weight, &[const_0u, weight_idx]);
    ...
    // k += 1
    let new_k = b.iadd(ty_uint, phi_k, const_1u);

    // Fixup phi nodes with loop continue values.
    fixup_phi(&mut b.functions, phi_k, new_k, loop_continue);
    ...
    // Add bias: linear_out = acc + bias[col]
    let ptr_bias = b.access_chain(ptr_sb_float, var_bias, &[const_0u, col]);
    let bias_val = b.load(ty_float, ptr_bias);
    let linear_out = b.fadd(ty_float, phi_acc, bias_val);

    // Apply activation.
    let activated = match activation {
        ActivationKind::ReLU => {
            // ReLU: max(0, linear_out)
            b.ext_inst(
                ty_float,
                glsl_ext,
                GLSL_STD_450_FMAX,
                &[const_zero_f, linear_out],
            )
        }
        ActivationKind::Gelu => {
            ...
            let x2 = b.fmul(ty_float, linear_out, linear_out);
            let x3 = b.fmul(ty_float, x2, linear_out);
            let coeff_x3 = b.fmul(ty_float, const_coeff, x3);
            let inner = b.fadd(ty_float, linear_out, coeff_x3);
            ...
            let half_x = b.fmul(ty_float, const_half, linear_out);
            b.fmul(ty_float, half_x, one_plus_tanh)
        }
        ActivationKind::SiLU => {
            // SiLU: x / (1 + exp(-x))
            let const_one = b.constant_f32(ty_float, 1.0);
            let const_neg_one = b.constant_f32(ty_float, -1.0);
            let neg_x = b.fmul(ty_float, const_neg_one, linear_out);
            let exp_neg = b.ext_inst(ty_float, glsl_ext, GLSL_STD_450_EXP, &[neg_x]);
            let denom = b.fadd(ty_float, const_one, exp_neg);
            b.fdiv(ty_float, linear_out, denom)
        }
    };

    // Store output[gid_x] = activated.
    let ptr_out = b.access_chain(ptr_sb_float, var_output, &[const_0u, gid_x]);
    b.store(ptr_out, activated);

    b.branch(label_exit);
    b.label_with_id(label_exit);
    b.op_return();
    b.func_end();
...
/// Generate a SPIR-V 1.0 binary for fused Linear + ReLU.
///
/// Each thread computes:
/// `output[row, col] = max(0, dot(input[row, :], weight[col, :]) + bias[col])`
///
/// # Arguments
///
...
pub fn generate_fused_linear_relu_spirv(in_features: u32, out_features: u32) -> Vec<u32> {
    let _ = (in_features, out_features);
    generate_fused_linear_act_inner(ActivationKind::ReLU)
}

/// Generate a SPIR-V 1.0 binary for fused Linear + GELU (tanh approximation).
///
/// Each thread computes:
/// `output[row, col] = GELU(dot(input[row, :], weight[col, :]) + bias[col])`
///
/// # Arguments
///
/// * `in_features` - Input dimension (compile-time hint; actual from push constants).
...
///
/// SPIR-V binary as `Vec<u32>`.
pub fn generate_fused_linear_gelu_spirv(in_features: u32, out_features: u32) -> Vec<u32> {
    let _ = (in_features, out_features);
    generate_fused_linear_act_inner(ActivationKind::Gelu)
}

/// Generate a SPIR-V 1.0 binary for fused Linear + SiLU/Swish.
///
/// Each thread computes:
/// `output[row, col] = SiLU(dot(input[row, :], weight[col, :]) + bias[col])`
///
/// # Arguments
///
/// * `in_features` - Input dimension (compile-time hint; actual from push constants).
...
///
/// SPIR-V binary as `Vec<u32>`.
pub fn generate_fused_linear_silu_spirv(in_features: u32, out_features: u32) -> Vec<u32> {
    let _ = (in_features, out_features);
    generate_fused_linear_act_inner(ActivationKind::SiLU)
}
...
/// Reference fused Linear + ReLU on CPU.
///
/// `output[row, col] = max(0, sum_k(input[row*in_f + k] * weight[col*in_f + k]) + bias[col])`
pub fn fused_linear_relu_reference(
    input: &[f32],
    weight: &[f32],
    bias: &[f32],
    batch_size: usize,
    ...
    for row in 0..batch_size {
        for col in 0..out_features {
            ...
            acc += bias[col];
            output[row * out_features + col] = acc.max(0.0);
        }
    }
    output
}

/// Reference fused Linear + GELU on CPU (tanh approximation).
pub fn fused_linear_gelu_reference(
    input: &[f32],
    weight: &[f32],
    bias: &[f32],
    batch_size: usize,
    ...
    for row in 0..batch_size {
        for col in 0..out_features {
            ...
            acc += bias[col];
            let inner = acc + 0.044715 * acc * acc * acc;
            output[row * out_features + col] = 0.5 * acc * (1.0 + (sqrt_2_over_pi * inner).tanh());
        }
    }
    output
}

/// Reference fused Linear + SiLU on CPU.
pub fn fused_linear_silu_reference(
    input: &[f32],
    weight: &[f32],
    bias: &[f32],
    batch_size: usize,
    ...
    for row in 0..batch_size {
        for col in 0..out_features {
            ...
            acc += bias[col];
            output[row * out_features + col] = acc / (1.0 + (-acc).exp());
        }
    }
    output
}
...
#[cfg(test)]
#[path = "spirv_fused_linear_act_tests.rs"]
mod spirv_fused_linear_act_tests;
