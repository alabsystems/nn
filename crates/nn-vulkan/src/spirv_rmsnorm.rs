...
//! Dedicated SPIR-V binary generation for RMSNorm compute shaders.
//!
//! Generates a SPIR-V 1.0 binary module for row-wise RMSNorm with separate
//! input, weight, and output buffers:
//!
//!   RMSNorm(x) = x * rsqrt(mean(x^2) + eps) * weight
//!
//! The kernel uses two phases with workgroup shared memory:
//!
//! 1. **Sum-of-squares reduction**: Each thread accumulates a partial sum of
//!    x^2 over a stripe of the row, then a tree reduction in shared memory
//!    finds the global sum. The mean is computed as sum / N.
//! 2. **Normalize + weight**: Each thread computes `x * rsqrt(mean_sq + eps) * weight`
//!    for its stripe elements and writes to the output buffer.
//!
//! Unlike the RMSNorm in [`super::spirv_layernorm`], this module uses a
//! **dedicated configuration struct** and **separate input/weight/output buffers**
//! (binding 0 = input, binding 1 = weight, binding 2 = output), matching the
//! pattern used by the separate-I/O softmax kernel.
//!
...
/// Workgroup size for RMSNorm kernels (1D dispatch).
pub const RMSNORM_WORKGROUP_SIZE: u32 = 256;
...
/// Configuration for RMSNorm SPIR-V kernel generation.
#[derive(Debug, Clone)]
pub struct RmsNormConfig {
    /// Number of elements in the normalized dimension (compile-time hint;
    /// actual value comes from push constants at runtime).
    pub hidden_dim: u32,
    /// Epsilon for numerical stability, baked into the shader as a constant.
    pub eps: f32,
    /// Entry point name for the generated SPIR-V module.
    pub kernel_name: String,
}

impl RmsNormConfig {
    /// Create a new configuration with the given hidden dimension and epsilon.
    ///
    /// Uses "main" as the default kernel name.
    pub fn new(hidden_dim: u32, eps: f32) -> Self {
        Self {
            hidden_dim,
            eps,
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if !self.eps.is_finite() || self.eps <= 0.0 {
            return Err(format!(
                "eps must be a finite positive value, got {}",
                self.eps
            ));
        }
        if self.kernel_name.is_empty() {
            return Err("kernel_name must not be empty".to_string());
        }
        Ok(())
    }
}
...
/// Generate a SPIR-V 1.0 binary module for RMSNorm with separate I/O buffers.
///
/// The kernel computes `RMSNorm(x) = x * rsqrt(mean(x^2) + eps) * weight`
/// along the last axis of a `[rows, hidden_dim]` matrix. Each workgroup
/// handles one row. The dispatch should be `(rows, 1, 1)` workgroups.
///
/// # Layout
///
/// - **Binding 0** (set 0): Input buffer `float[]` (readonly).
/// - **Binding 1** (set 0): Weight buffer `float[hidden_dim]` (readonly).
/// - **Binding 2** (set 0): Output buffer `float[]` (writeonly).
/// - **Push constants**: `{ uint total_elements; uint hidden_dim; }`.
/// - **Shared memory**: `float[RMSNORM_WORKGROUP_SIZE]` for reductions.
///
/// # Arguments
///
/// * `config` — Configuration specifying hidden dimension, epsilon, and kernel name.
///
/// # Example
///
/// ```
/// use nn_vulkan::spirv_rmsnorm::{RmsNormConfig, generate_rmsnorm_separate_io_spirv};
/// let config = RmsNormConfig::new(768, 1e-5);
/// let spirv = generate_rmsnorm_separate_io_spirv(&config);
/// assert_eq!(spirv.len() % 4, 0);
/// let magic = u32::from_le_bytes([spirv[0], spirv[1], spirv[2], spirv[3]]);
/// assert_eq!(magic, 0x07230203);
/// ```
pub fn generate_rmsnorm_separate_io_spirv(config: &RmsNormConfig) -> Vec<u8> {
    let _ = config.hidden_dim; // hint; actual dim from push constants.
    ...
    // Weight buffer struct (readonly, binding 1).
    ...
    // Output buffer struct (writeonly, binding 2).
    ...
    // Push constant struct: { uint total_elements; uint hidden_dim; }
    ...
    // Shared memory: float[RMSNORM_WORKGROUP_SIZE]
    let const_wg_size = b.constant_u32(ty_uint, RMSNORM_WORKGROUP_SIZE);
    ...
    let const_eps = b.constant_f32(ty_float, config.eps);
    let const_half_wg = b.constant_u32(ty_uint, RMSNORM_WORKGROUP_SIZE / 2);
    ...
    // Entry point.
    b.entry_point_compute(func_id, &config.kernel_name, &[var_gid, var_lid]);
    b.execution_mode_local_size(func_id, RMSNORM_WORKGROUP_SIZE, 1, 1);
    ...
    let pc_hidden_ptr = b.access_chain(ptr_pc_uint, var_pc, &[const_1u]);
    let dim_hidden = b.load(ty_uint, pc_hidden_ptr);
    ...
    // Base offset for this row: row_base = row * hidden_dim.
    let row_base = b.imul(ty_uint, row, dim_hidden);

    // ================================================================
    // Phase 1: Compute sum(x^2) via shared memory reduction.
    // ================================================================
    ...
    let cmp_col1 = b.u_less_than(ty_bool, phi_col1, dim_hidden);
    ...
    // Load sq sum from shared[0] and compute inv_rms = 1 / sqrt(mean(x^2) + eps).
    let ptr_s0 = b.access_chain(ptr_wg_float, var_shared, &[const_0u]);
    let sq_sum_total = b.load(ty_float, ptr_s0);
    let n_float = b.convert_u_to_f(ty_float, dim_hidden);
    ...
    // ================================================================
    // Phase 2: Normalize and apply weight: out = x * inv_rms * weight
    // ================================================================
    ...
    let cmp_col2 = b.u_less_than(ty_bool, phi_col2, dim_hidden);
    ...
    let result_val = b.fmul(ty_float, w_val, normalized);
...
/// Compute reference RMSNorm on CPU for testing.
///
/// Computes row-wise RMSNorm: `out_i = x_i * rsqrt(mean(x^2) + eps) * weight_i`.
/// Returns a flat `Vec<f32>` with the same layout as input.
///
/// # Arguments
///
/// * `input` — Flat input tensor of shape `[rows, hidden_dim]`.
/// * `weight` — Weight/scale vector of shape `[hidden_dim]`.
/// * `rows` — Number of rows.
...
pub fn rmsnorm_reference(
    ...
    assert_eq!(
        input.len(),
        rows * hidden_dim,
        "input length must be rows * hidden_dim"
    );
    assert_eq!(weight.len(), hidden_dim, "weight length must be hidden_dim");

    let mut output = vec![0.0f32; rows * hidden_dim];
    for row in 0..rows {
        let base = row * hidden_dim;
        let row_slice = &input[base..base + hidden_dim];

        // Phase 1: Compute mean(x^2).
        let sq_sum: f32 = row_slice.iter().map(|&x| x * x).sum();
        let mean_sq = sq_sum / (hidden_dim as f32);

        // Phase 2: Normalize with rsqrt and apply weight.
        let inv_rms = 1.0 / (mean_sq + eps).sqrt();
        for j in 0..hidden_dim {
            output[base + j] = row_slice[j] * inv_rms * weight[j];
        }
    }
    output
}
...
#[cfg(test)]
#[path = "spirv_rmsnorm_tests.rs"]
mod spirv_rmsnorm_tests;
