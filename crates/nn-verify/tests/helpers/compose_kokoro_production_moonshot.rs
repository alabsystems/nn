...
fn test_production_moonshot_full_8_property_certificate() {
    ...
    // P8: ay BOUNDS_REGISTRY counts (20 proven kernels)
    let smt_evidence = nn_tts_verify::SmtVerificationEvidence {
        kernels_proven: 20,
        kernels_total: 20,
        proven_kernel_names: nn_tts_verify::moonshot_crown::ay_proven_kernel_names()
            .iter()
            .map(|s| s.to_string())
            .collect(),
        all_proven: true,
    ...
    // P8 must be SmtProven (20/20 BOUNDS_REGISTRY entries)
    assert_eq!(
...
fn test_production_moonshot_record_status() {
    ...
    let (in_lo, in_hi) = super::common::bounds_min_max(&te.input_bounds);
...
fn test_moonshot_d256_concentration_bridge() {
    ...
    let stage1 = nn_tts_verify::pipeline::stage_from_bounds(
        "text_encoder_d256",
        &super::common::uniform_bounds(&[dim], 1.0),
        &super::common::uniform_bounds(&[dim], 0.5),
        "IBP",
        false,
    );
    ...
    let pp_output = super::common::uniform_bounds(&[dim], 1.5);
    let stage2 = nn_tts_verify::pipeline::stage_from_bounds(
...
fn test_moonshot_d256_mcdiarmid_certificate() {
    ...
    let crown_bounds = super::common::uniform_bounds(&[dim], 1.5);
    let input_bounds = super::common::uniform_bounds(&[dim], 1.0);
...
fn test_moonshot_d256_distributional() {
    ...
    // Build a simple 2-layer linear network with near-identity weights.
    // All-linear → CROWN is exact (no relaxation gap).
    let mut network = Network::new();
    ...
    // prob bounds should be tighter than IBP interval bounds for a linear network
    // with well-behaved weights. The distributional bound uses mean +/- z*sqrt(var)
    // which for near-identity diagonal weights produces tighter ranges.
    ...
    eprintln!(
        "IBP total width: {ibp_width_sum:.2}, Dist total width: {dist_width_sum:.2}, \
         tighter: {}",
        if dist_width_sum <= ibp_width_sum {
            "YES"
        } else {
            "NO"
        }
    ...
    // Distributional and IBP bounds must both be finite. Distributional may
    // be wider when +1 axis convention causes bound widening (#2987).
    assert!(
        dist_width_sum.is_finite(),
        "distributional bounds must be finite, got {dist_width_sum}"
    );
    assert!(
        ibp_width_sum.is_finite(),
        "IBP bounds must be finite, got {ibp_width_sum}"
    );
    ...
    let result = nn_tts_verify::moonshot_crown::check_non_clipping_distributional(
        ...
        &input_bounds,
        confidence,
        true, // is_sound (all-linear network)
    );
    eprintln!(
    ...
    // The network maps [-1,1] through y = 0.6*(0.5*x + 0.1) = 0.3*x + 0.06.
    // Output range: [-0.3+0.06, 0.3+0.06] = [-0.24, 0.36].
    // At 99% confidence, distributional bounds are tighter → should prove non-clipping.
    assert!(
        result.proven,
        "distributional P2 should be proven for this linear network"
    );
}
...
fn test_moonshot_d512_concentration_bridge() {
    ...
    let pp_output = super::common::uniform_bounds(&[dim], 2.0);
    let stage2 = nn_tts_verify::pipeline::stage_from_bounds(
...
fn test_production_moonshot_3stage_te_pp_f0() {
    ...
    match f0_result {
        Ok(f0) => {
            ...
            super::kokoro_production_weights::record_segment(
...
///
/// Both F0 and Generator may fail (F0: ConvTranspose1d, Generator: v1.0
/// architecture mismatch). The test is structured to verify the longest
/// pipeline that succeeds: 4-stage > 3-stage > 2-stage.
///
...
fn test_production_moonshot_4stage_full_pipeline() {
    ...
    if let Ok(ref gstage) = generator_result {
        let (gen_lo, gen_hi) = super::common::bounds_min_max(&gstage.output_bounds);
    ...
    let (in_lo, in_hi) = super::common::bounds_min_max(&te.input_bounds);
    let (out_lo, out_hi) = super::common::bounds_min_max(final_bounds);
