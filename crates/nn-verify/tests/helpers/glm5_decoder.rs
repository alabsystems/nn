...
//! Builder helpers for GLM-5 decoder NY composition tests.
//!
//! Architecture: Embedding -> N x (RmsNorm + MHA + residual, RmsNorm + SwiGLU + residual)
//! -> RmsNorm -> Linear (output_layer).
//!
//! GLM-5 specifics (from `crates/nn-glm5/src/layers.rs`):
...
//!
//! Dimensions for test tractability (production GLM-4-9B in parens):
//! - D_MODEL=32 (4096), N_HEADS=4 (32), HEAD_DIM=8 (128)
//! - FFN_DIM=128 (13696), VOCAB_SIZE=16 (151552), SEQ_LEN=4 (8192)
...
/// Number of decoder layers for the 2-block stack test.
...
// ---------------------------------------------------------------------------
// 1. Self-attention sub-graph builder
// ---------------------------------------------------------------------------

/// Build a GLM-5 self-attention sub-graph.
///
/// Input: `[SEQ_LEN, D_MODEL]`.
/// Output: `[SEQ_LEN, D_MODEL]`.
///
/// Pipeline (mirrors `Glm5Attention::forward`):
///   1. RmsNorm(input)
...
pub(super) fn build_glm5_self_attention() -> TensorKernelDef {
    let mut b = TensorBlockBuilder::new("glm5_self_attention");
    ...
    b.build(output)
        .expect("valid GLM-5 self-attention sub-graph")
}

/// Bindings for GLM-5 self-attention: hidden=Variable, rest=Constant.
pub(super) fn glm5_self_attention_bindings() -> Vec<TensorParamBinding> {
...
// ---------------------------------------------------------------------------
// 2. SwiGLU MLP sub-graph builder
// ---------------------------------------------------------------------------

/// Build a GLM-5 SwiGLU MLP sub-graph.
///
/// Input: `[SEQ_LEN, D_MODEL]`.
/// Output: `[SEQ_LEN, D_MODEL]`.
///
/// Pipeline (mirrors `Glm5MLP::forward`):
...
pub(super) fn build_glm5_swiglu_ffn() -> TensorKernelDef {
    let mut b = TensorBlockBuilder::new("glm5_swiglu_ffn");
    ...
    b.build(out).expect("valid GLM-5 SwiGLU FFN sub-graph")
}

/// Bindings for GLM-5 SwiGLU FFN: hidden=Variable, weights=Constant.
pub(super) fn glm5_swiglu_ffn_bindings() -> Vec<TensorParamBinding> {
    vec![
...
// ---------------------------------------------------------------------------
// 3. RMSNorm sub-graph builder
// ---------------------------------------------------------------------------

/// Build a standalone RMSNorm sub-graph for GLM-5.
///
/// Input: `[SEQ_LEN, D_MODEL]`.
/// Output: `[SEQ_LEN, D_MODEL]`.
pub(super) fn build_glm5_rmsnorm() -> TensorKernelDef {
    let mut b = TensorBlockBuilder::new("glm5_rmsnorm");
    let shape = [SEQ_LEN, D_MODEL];
    ...
    b.build(output).expect("valid GLM-5 RMSNorm sub-graph")
}

/// Bindings for GLM-5 RMSNorm: hidden=Variable, eps+weight=Constant.
pub(super) fn glm5_rmsnorm_bindings() -> Vec<TensorParamBinding> {
    vec![
        TensorParamBinding::Variable,             // hidden
        TensorParamBinding::ConstantScalar(1e-5), // eps
        TensorParamBinding::ConstantTensor(ArrayD::from_elem(IxDyn(&[D_MODEL]), 1.0f32)), // weight (scale)
    ]
}

// ---------------------------------------------------------------------------
// Full decoder block helper (shared by block + stack builders)
// ---------------------------------------------------------------------------

/// Weights for a single GLM-5 decoder block.
struct Glm5BlockWeights {
    // Attention sub-block
...
/// Build a GLM-5 SwiGLU FFN within a decoder block using pre-allocated weights.
...
/// Build a single GLM-5 decoder block.
///
...
// ---------------------------------------------------------------------------
// 4. Single decoder block as standalone TensorKernelDef
// ---------------------------------------------------------------------------

/// Build a single GLM-5 decoder block as a standalone `TensorKernelDef`.
pub(super) fn build_glm5_decoder_block() -> TensorKernelDef {
    let mut b = TensorBlockBuilder::new("glm5_decoder_block");
    ...
    b.build(output)
        .expect("valid GLM-5 decoder block sub-graph")
}

/// Bindings for a single GLM-5 decoder block: hidden=Variable, rest=Constant.
#[allow(clippy::vec_init_then_push)]
pub(super) fn glm5_decoder_block_bindings() -> Vec<TensorParamBinding> {
    let mut bindings = Vec::new();
...
// ---------------------------------------------------------------------------
// 5. 2-block decoder stack
// ---------------------------------------------------------------------------

/// Build a 2-block GLM-5 decoder stack as a `TensorKernelDef`.
///
/// Architecture: hidden -> 2 x DecoderBlock -> RmsNorm -> lm_head.
pub(super) fn build_glm5_decoder_stack() -> TensorKernelDef {
    let mut b = TensorBlockBuilder::new("glm5_decoder_stack_2");
    ...
    b.build(logits)
        .expect("valid GLM-5 decoder stack sub-graph")
}

/// Bindings for 2-block GLM-5 decoder stack: hidden=Variable, rest=Constant.
#[allow(clippy::vec_init_then_push)]
pub(super) fn glm5_decoder_stack_bindings() -> Vec<TensorParamBinding> {
    let mut bindings = Vec::new();
...
// ---------------------------------------------------------------------------
// 6. Full pipeline: embedding -> decoder stack -> lm_head
// ---------------------------------------------------------------------------

/// Build the full GLM-5 embedding-to-logits pipeline.
///
/// Architecture: Linear(embedding) -> 2 x DecoderBlock -> RmsNorm -> lm_head.
///
/// Uses a linear embedding projection as NY approximation of the
/// lookup-table embedding (non-differentiable discrete operation).
pub(super) fn build_glm5_embedding_to_logits() -> TensorKernelDef {
    let mut b = TensorBlockBuilder::new("glm5_embedding_to_logits");

    // Embedding input: [SEQ_LEN, D_MODEL] (post-embedding continuous representation)
    let input = b.add_input("embedded", &[SEQ_LEN, D_MODEL]);
    let eps = b.add_input("eps", &[1]);
    ...
    b.build(logits)
        .expect("valid GLM-5 embedding-to-logits pipeline")
}

/// Bindings for GLM-5 embedding-to-logits: embedded=Variable, rest=Constant.
///
/// Same structure as decoder_stack_bindings (the embedding is the Variable input).
pub(super) fn glm5_embedding_to_logits_bindings() -> Vec<TensorParamBinding> {
    // Identical to decoder stack bindings -- the Variable input represents
    // post-embedding hidden states in both cases.
    glm5_decoder_stack_bindings()
}
