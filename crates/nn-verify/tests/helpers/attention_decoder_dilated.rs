...
//! Phase 30 helpers: dilated convolution in ResBlocks.
//!
//! Extends Phase 29's multi-stage decoder to use dilated convolutions
//! within each ResBlock, matching Kokoro ISTFTNet's actual architecture:
//!
//!   ResBlock = for d in dilations:
//!     IN1 → Snake(α1) → Conv1d(dilation=d) → IN2 → Snake(α2) → Conv1d(dilation=1) → residual add
//!
//! Production Kokoro uses dilations=[1,3,5] per ResBlock. Each dilated
//! conv uses same-padding: padding = dilation*(kernel-1)/2 to preserve
//! temporal length.
//!
//! Part of #1729: Attention Monotonicity Proofs — Phase 30.
...
use super::common::decoder_common::{
    ...
    AttnLayerIds, DilatedSubLayerIds, D_K, D_MODEL, FFN_DIM, NUM_HEADS, OUT_KERNEL, OUT_PADDING,
    T_DEC, T_ENC, UPSAMPLE_KERNEL, UPSAMPLE_PADDING, UPSAMPLE_STRIDE, WEIGHT_MAG,
};
use super::common::weights;

// Re-export items used by test files via `attn_decoder_dilated::`.
pub(super) use super::common::decoder_common::KOKORO_DILATIONS;

const RESBLOCK_KERNEL: usize = 3;

struct StageIds {
    ups_w: TensorNodeId,
    sublayers: Vec<DilatedSubLayerIds>,
}

// -------------------------------------------------------------------------
// Dilated ResBlock pipeline builder
// -------------------------------------------------------------------------

/// Build attention → bridge → multi-stage decoder with dilated ResBlocks.
///
/// `na`: attention layers (≥2).
/// `ns`: upsample stages (1-3).
/// `dilations`: dilation pattern per ResBlock (e.g., [1, 3, 5]).
///
/// Each sub-layer in the ResBlock applies:
///   IN → Snake → DilatedConv → IN → Snake → Conv(d=1) → residual add
///
/// Returns (kernel_def, output_shape).
pub(super) fn build_dilated_pipeline(
    na: usize,
    ns: usize,
    dilations: &[usize],
) -> (TensorKernelDef, [usize; 2]) {
    assert!(na >= 2 && (1..=3).contains(&ns) && !dilations.is_empty());

    let scale = 1.0 / (D_K as f32).sqrt();
    let ss = [NUM_HEADS, T_DEC, T_ENC];
    let cs = [NUM_HEADS, T_DEC, D_K];

    let mut b = TensorBlockBuilder::new("dilated_pipeline");
    ...
    for si in 0..ns {
        ...
        for (di, &_dil) in dilations.iter().enumerate() {
            ...
            sublayers.push(DilatedSubLayerIds {
                ...
                conv_w: b.add_input(&format!("cd{ds}"), &[out_ch, out_ch, RESBLOCK_KERNEL]),
                gamma2: b.add_input(&format!("g2{ds}"), &[out_ch]),
                beta2: b.add_input(&format!("b2{ds}"), &[out_ch]),
                alpha2: b.add_input(&format!("a2{ds}"), &[1]),
                conv_unit_w: b.add_input(&format!("cu{ds}"), &[out_ch, out_ch, RESBLOCK_KERNEL]),
            });
        }

        stage_ids.push(StageIds { ups_w, sublayers });
    }
    ...
    // === Multi-Stage Decoder with Dilated ResBlocks ===
    ...
    for (si, stage) in stage_ids.iter().enumerate() {
        ...
        // Dilated ResBlock sub-layers
        for (di_idx, sl) in stage.sublayers.iter().enumerate() {
            let dilation = dilations[di_idx];

            // Sub-layer 1: IN → Snake → DilatedConv1d
            let n1 =
                b.add_instance_norm(cur, dec_eps, 1, Some(sl.gamma1), Some(sl.beta1), &up_shape);
            let a1bc = b.add_broadcast(sl.alpha1, &up_shape);
            let sn1 = b.add_elementwise(sk.clone(), &[n1, a1bc], &up_shape);

            let dil_pad = dilated_same_padding(RESBLOCK_KERNEL, dilation);
            let cv1 = b.add_conv1d_full(
                sn1, sl.conv_w, None, 1,        // stride
                dil_pad,  // padding for same-length output
                dilation, // dilated conv
                1,        // groups
                &up_shape,
            );

            // Sub-layer 2: IN → Snake → Conv1d(dilation=1)
            let n2 =
                b.add_instance_norm(cv1, dec_eps, 1, Some(sl.gamma2), Some(sl.beta2), &up_shape);
            let a2bc = b.add_broadcast(sl.alpha2, &up_shape);
            let sn2 = b.add_elementwise(sk.clone(), &[n2, a2bc], &up_shape);

            let unit_pad = dilated_same_padding(RESBLOCK_KERNEL, 1);
            ...
            // Residual
            cur = b.add_binary_add(cur, cv2, &up_shape);
        }
    ...
    (
        b.build(out).expect("valid dilated pipeline"),
        [final_ch, final_t],
    )
}
...
/// Build bindings for the dilated pipeline.
pub(super) fn dilated_bindings(
    na: usize,
    ns: usize,
    dilations: &[usize],
    pe_scale: f32,
...
#[derive(Debug)]
pub(super) struct DilatedResult {
...
pub(super) fn analyze_dilated_output(
