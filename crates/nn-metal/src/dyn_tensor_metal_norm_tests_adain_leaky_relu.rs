...
//! Tests for fused AdaIN+LeakyRelu GPU kernel (#2472).
//!
//! Verifies that the fused GPU dispatch (InstanceNorm → affine → LeakyRelu)
...
/// Run AdaIn::forward_leaky_relu on both CPU and GPU, compare results.
fn assert_adain_leaky_relu_gpu_matches_cpu(
    channels: usize,
    time: usize,
    style_dim: usize,
    slope: f64,
...
#[test]
fn test_fused_adain_leaky_relu_basic() {
    // Minimal: 4 channels, 8 time steps, style_dim=8, slope=0.2
    assert_adain_leaky_relu_gpu_matches_cpu(4, 8, 8, 0.2, 1e-4, "adain_leaky_relu_basic");
}

#[test]
fn test_fused_adain_leaky_relu_kokoro_like() {
    // Kokoro F0EnergyPredictor-like: 64 channels, 128 time steps, style_dim=128
    assert_adain_leaky_relu_gpu_matches_cpu(
        64,
        128,
        128,
        0.2,
        1e-3,
        "adain_leaky_relu_kokoro_like",
    );
}

#[test]
fn test_fused_adain_leaky_relu_single_channel() {
    // Edge case: 1 channel
    assert_adain_leaky_relu_gpu_matches_cpu(1, 16, 4, 0.2, 1e-4, "adain_leaky_relu_single_ch");
}

#[test]
fn test_fused_adain_leaky_relu_zero_slope() {
    // ReLU case: slope=0.0 (standard ReLU, not leaky)
    assert_adain_leaky_relu_gpu_matches_cpu(4, 16, 8, 0.0, 1e-4, "adain_leaky_relu_zero_slope");
}
