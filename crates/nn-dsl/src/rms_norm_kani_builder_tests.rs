...
//! Kani proof harnesses for `build_rms_norm_decomposed`.
//!
...
//!
//! Proves structural correctness of the decomposed RMSNorm builder:
...
use super::build_rms_norm_decomposed;

/// Proves `build_rms_norm_decomposed` does not panic for any bounded
/// parameter values, including invalid ones (0 dimensions) that return `Err`.
#[kani::unwind(1)]
#[kani::proof]
fn rms_norm_decomposed_build_no_panic() {
    ...
    let _ = build_rms_norm_decomposed(n, hidden);
}

/// Proves the output shape matches the input shape `[N, hidden]`.
///
/// RMSNorm is shape-preserving: `x * rsqrt(mean(x²) + eps) * weight` has the
/// same shape as `x`.
#[kani::unwind(1)]
#[kani::proof]
fn rms_norm_decomposed_output_shape_preserved() {
    let n: usize = kani::any();
    let hidden: usize = kani::any();
    ...
    let def = build_rms_norm_decomposed(n, hidden).expect("valid params must succeed");
    ...
    assert_eq!(
        ...
        "RMSNorm output shape must equal input shape [N, hidden]"
    );
...
/// Proves `build_rms_norm_decomposed` rejects zero-dimension inputs
...
#[kani::unwind(1)]
#[kani::proof]
fn rms_norm_decomposed_zero_dim_returns_err() {
    let n: usize = kani::any();
    let hidden: usize = kani::any();
    ...
    let result = build_rms_norm_decomposed(n, hidden);
    assert!(
        result.is_err(),
        "zero-dimension RMSNorm must return Err, not panic"
    );
}
