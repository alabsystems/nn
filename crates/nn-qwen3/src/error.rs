...
//! Error types for the Qwen3 LLM model.

use nn_core::{BackendDomain, BackendErrorKind, TensorError};

/// Errors from Qwen3 model construction or forward pass.
...
impl From<Qwen3Error> for TensorError {
    fn from(e: Qwen3Error) -> Self {
        match e {
            Qwen3Error::Tensor(te) => te,
            other => {
